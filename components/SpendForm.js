...
export default function SpendForm({ onSubmit, initialData }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {tools.map((tool) => (
            <div key={tool.id} className="border border-gray-200 rounded-lg p-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <select required value={tool.name} onChange={(e) => updateTool(tool.id, 'name', e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg">
                  <option value="">Select tool</option>
                  {availableTools.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
                </select>
                <select required value={tool.plan} onChange={(e) => updateTool(tool.id, 'plan', e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg" disabled={!tool.name}>
                  <option value="">Select plan</option>
                  {tool.name && getPlansForTool(tool.name).map(plan => <option key={plan} value={plan}>{plan}</option>)}
                </select>
                <input type="number" required placeholder="Monthly spend ($)" value={tool.monthlySpend} onChange={(e) => updateTool(tool.id, 'monthlySpend', e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg" />
                <input type="number" required min="1" placeholder="Seats" value={tool.seats} onChange={(e) => updateTool(tool.id, 'seats', e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg" />
              </div>
      ...
      <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors disabled:bg-gray-400">
